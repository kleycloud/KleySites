...
import { sanearHTML, escapeHTML } from './sanear.js';
...
const RENDERERS = {
  ...
  boton: (c, s, op) => {
    ...
    return `<a href="${escapeHTML(c.href || '#')}" style="${BOTON_BASE};${s}">${texto}</a>`;
  },
  ...
  // los hijos y el wrapper aplica el estilo.
  seccion: (c, s, op, hijos) => (op.lienzo ? hijos : `<section style="${s}">${hijos}</section>`),
...
export function renderBloque(b, hijos = '', opciones = {}) {
  ...
  if (!render) return '';
  return render(b.contenido || {}, estilosACSS(b.estilos, b.tipo), opciones, hijos);
}
