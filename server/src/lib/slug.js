...
export async function slugDisponible(c, base) {
  const random = Math.floor(100 + Math.random() * 900);
  const candidatos = [base, `${base}-pe`, `${base}-oficial`, `${base}-${random}`, `${base}-2`];
  const { rows } = await consultar(c, 'SELECT slug FROM sites WHERE slug = ANY($1)', [candidatos]);
  const ocupados = new Set(rows.map((r) => r.slug));
  return candidatos.find((c2) => !ocupados.has(c2)) || `${base}-${Date.now()}`;
}
