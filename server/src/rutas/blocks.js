...
// mande el navegador).
async function pageEsDelCliente(c, pageId, clientId) {
...
app.get('/', async (c) => {
  ...
  if (!pageId) return error(c, 400, 'Falta page_id.');
  if (!(await pageEsDelCliente(c, pageId, client_id))) return error(c, 404, 'Página no encontrada.');
  ...
  return c.json({ bloques: rows });
});

app.post('/', async (c) => {
  const { client_id } = await clienteDesdeContexto(c);
  const body = await c.req.json();
  if (!body.page_id) return error(c, 400, 'Falta page_id.');
  if (!(await pageEsDelCliente(c, body.page_id, client_id))) return error(c, 404, 'Página no encontrada.');
  ...
  return c.json({ bloque: rows[0] });
});

app.delete('/', async (c) => {
  const { client_id } = await clienteDesdeContexto(c);
  const blockId = c.req.query('block_id');
  if (!blockId) return error(c, 400, 'Falta block_id.');
  ...
  if (!rows[0]) return error(c, 404, 'Bloque no encontrado.');
